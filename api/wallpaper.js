...
`;
  ...
  // Background geometric tile
  {
    ...
    for (let gx = PS / 2; gx < W + PS; gx += PS)
      for (let gy = PS / 2; gy < H * 0.55; gy += PS)
        o += `<polygon points="${star8(gx, gy, PS * 0.4, PS * 0.18)}"/>`;
    o += `</g>`;
  }
  ...
  <circle cx="${(MX + MR * 0.54).toFixed(1)}" cy="${(MY - MR * 0.2).toFixed(1)}" r="${(MR * 0.79).toFixed(1)}" fill="${sky[0]}"/>
</g>`;
    const decorStars = [
      [W * 0.84, H * 0.048, W * 0.009, 0.55],
      [W * 0.62, H * 0.04, W * 0.007, 0.45],
      [W * 0.8, H * 0.028, W * 0.006, 0.38],
    ];
    for (const [sx, sy, sr, so] of decorStars) {
      ...
      for (let i = 0; i < 16; i++) {
        const r = i % 2 === 0 ? sr : sr * 0.42,
          a = (i * Math.PI) / 8 - Math.PI / 2;
        p += `${(sx + r * Math.cos(a)).toFixed(1)},${(sy + r * Math.sin(a)).toFixed(1)} `;
      }
  ...
  for (const d of dots) {
    if (d.day < ramadanDay) {
      ...
      o += `<circle cx="${d.x.toFixed(1)}" cy="${d.y.toFixed(1)}" r="${(W * 0.01).toFixed(1)}" fill="rgba(255,255,255,0.03)" stroke="rgba(212,168,71,0.20)" stroke-width="1"/>`;
    }
  }
  ...
  // DIVIDER
  {
    ...
    const sr1 = W * 0.006,
      sr2 = sr1 * 0.42;
    for (let i = 0; i < 16; i++) {
      const r = i % 2 === 0 ? sr1 : sr2,
        a = (i * Math.PI) / 8 - Math.PI / 2;
      sp += `${(W / 2 + r * Math.cos(a)).toFixed(1)},${(parseFloat(DY) + r * Math.sin(a)).toFixed(1)} `;
    }
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const errorSvg = `<svg xmlns="http://www.w3.org/2000/svg" width="400" height="120">
      ...
      <text x="200" y="52" text-anchor="middle" fill="#D4A847" font-size="14">Error</text>
      <text x="200" y="80" text-anchor="middle" fill="rgba(255,255,255,0.4)" font-size="11">${xe(err.message || "Unknown error")}</text>
    </svg>`;
